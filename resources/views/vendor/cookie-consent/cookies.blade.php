{{-- OfficeTalk Cookie-Consent-Banner · editorial Überarbeitung im Site-Stil.
     Banner sitzt auf bg-surface-strong (in Light + Dark Mode dunkel),
     deshalb alle Textfarben theme-stabil auf white-Basis halten. --}}
<aside
    id="cookies-policy"
    data-officetalk-consent
    class="officetalk-consent cookies cookies--no-js fixed inset-x-0 bottom-0 z-[90] bg-surface-strong text-white shadow-[0_-16px_48px_-16px_rgba(17,17,17,0.3)]"
...
>
    <div class="cookies__alert">
        <div class="cookies__container mx-auto max-w-[1280px] px-s3 py-s4 md:px-s5 md:py-s5">
            <div class="cookies__wrapper grid gap-s4 md:grid-cols-12 md:items-center md:gap-s5">
                ...
                <div class="md:col-span-7 lg:col-span-8">
                    ...
                    <div class="cookies__intro mt-s2 font-sans text-body leading-relaxed text-white/90">
                        <p>@lang('cookieConsent::cookies.intro')</p>
                        @if($policy)
                            <p class="mt-s1 font-sans text-meta text-white/70">
        ...
        <a
            ...
            class="cookies__btn cookies__btn--customize mx-auto flex max-w-[1280px] items-center justify-between gap-s2 border-t border-white/15 px-s3 py-s3 font-sans text-meta text-white/80 transition-colors duration-200 hover:text-accent md:px-s5"
        >
        ...
        <div
            class="cookies__expandable cookies__expandable--custom border-t border-white/15 bg-surface-strong"
            ...
            >
                ...
                <div class="cookies__sections grid gap-s4 md:grid-cols-2">
                    @foreach($cookies->getCategories() as $category)
                        <div class="cookies__section border-t border-white/20 pt-s3">
                            <label for="cookies-policy-check-{{ $category->key() }}" class="cookies__category flex cursor-pointer flex-col gap-s2">
                                <div class="flex items-start gap-s2">
                                    ...
                                    <span class="cookies__box">
                                        <strong class="cookies__label font-display text-h4 font-medium not-italic text-white">
                                            {{ $category->title }}
                                        </strong>
                                    </span>
                                </div>
                                ...
                                @if($category->description)
                                    <p class="cookies__info font-sans text-meta leading-relaxed text-white/75 md:pl-s5">
                            ...
                            <div class="cookies__expandable" id="cookies-policy-{{ $category->key() }}">
                                <ul class="cookies__definitions mt-s3 space-y-s2 border-t border-white/15 pt-s3 md:ml-s5">
                                    @foreach($category->getCookies() as $cookie)
                                        <li class="cookies__cookie">
                                            <p class="cookies__name font-sans text-meta font-semibold text-white">{{ $cookie->name }}</p>
                                            <p class="cookies__duration font-sans text-eyebrow uppercase tracking-[0.08em] text-white/60">
                                                {{ Carbon\Carbon::now()->diffForHumans(Carbon\Carbon::now()->addMinutes($cookie->duration), true) }}
                                            </p>
                                            @if($cookie->description)
                                                <p class="cookies__description mt-s1 font-sans text-meta leading-relaxed text-white/75">
                                                    {{ $cookie->description }}
                                                </p>
                                            @endif
                                        </li>
                            ...
                            <a
                                ...
                                class="cookies__details mt-s2 inline-block font-sans text-meta text-white/70 underline underline-offset-2 transition-colors duration-200 hover:text-accent md:ml-s5"
                            >
                ...
                <div class="cookies__save mt-s5 border-t border-white/20 pt-s4">
                    <button
                        type="submit"
...
<style data-cookie-consent>
    ...
    [data-officetalk-consent] .cookies__expandable:target {
        max-height: 2000px;
    }
    ...
    /* Outline-Variante · transparent mit heller Border · theme-stabil weiß */
    ...
    /* Stellt sicher, dass Paket-Default-Styles unsere Klassen nicht überschreiben */
